  ...
  setTimeout(() => {
    try {
      ...
      phaseElements.forEach(phaseElement => {
        // Extract phase number from the ID (assuming format with p1, p2, p3, etc.)
...
const phaseWeights = {
    ...
    'quick-start': { weeks: 2, name: 'Quick Start Track (Optional)' },
...
function updateProgressBar() {
    try {
        ...
        const documentEnd = documentHeight - windowHeight;

        // Find Phase 1 as our starting point (the H2 element, not SVG)
        const phase1Element = document.querySelector('h2#phase1, h3#phase1');
        if (!phase1Element) {
            console.warn('Phase 1 element not found');
            return;
        }

        const phase1Start = getElementOffset(phase1Element);

        // Build array of ALL phase positions that exist in the document
        // Use querySelector to find H2/H3 elements specifically, not SVG elements
        const phaseIds = Object.keys(phaseWeights);
        const phasePositions = [];

        for (let i = 0; i < phaseIds.length; i++) {
            const phaseId = phaseIds[i];
            // Find the H2 or H3 element with this ID, not SVG elements
            const phaseElement = document.querySelector(`h2#${phaseId}, h3#${phaseId}`);
            
            if (phaseElement) {
                const elementTop = getElementOffset(phaseElement);
                phasePositions.push({
                    id: phaseId,
                    top: elementTop,
                    weight: phaseWeights[phaseId].weeks || 0,
                    name: phaseWeights[phaseId].name
                });
            }
        }

        // Sort by position
        phasePositions.sort((a, b) => a.top - b.top);

        if (phasePositions.length === 0) {
            console.warn('No phase positions found');
            // Show progress based on simple scroll percentage
            const simpleProgress = Math.round((scrollTop / documentEnd) * 100);
            progressBar.style.width = simpleProgress + '%';
            progressPercent.textContent = simpleProgress + '%';
            progressPercent.classList.remove('hidden');
            return;
        }

        // Find the index of phase1 in our sorted array
        const phase1Index = phasePositions.findIndex(p => p.id === 'phase1');

        // If we haven't scrolled to phase 1 yet, show 0%
        if (phase1Index !== -1 && scrollTop < phase1Start) {
        ...
        // Calculate total weight from phase1 onwards
        const phasesFromPhase1 = phase1Index !== -1 ? phasePositions.slice(phase1Index) : phasePositions;
        const totalPhaseWeeks = phasesFromPhase1.reduce((sum, phase) => sum + phase.weight, 0);

        // Calculate the total scrollable distance from phase1 to document end
        const startPoint = phase1Index !== -1 ? phase1Start : phasePositions[0].top;
        const totalScrollDistance = documentEnd - startPoint;

        if (totalScrollDistance <= 0) {
            // Document too short, just show 100%
        ...
        // Calculate weighted progress (only for phases from phase1 onwards)
        let accumulatedWeight = 0;
        let currentPhase = null;

        // Check if we're at the very bottom first
        if (scrollTop >= documentEnd - 10) {
            accumulatedWeight = totalPhaseWeeks;
            currentPhase = phasesFromPhase1[phasesFromPhase1.length - 1].name;
        } else {
            // Calculate based on current position, only for phases from phase1 onwards
            for (let i = 0; i < phasesFromPhase1.length; i++) {
                const phase = phasesFromPhase1[i];
                const nextPhase = phasesFromPhase1[i + 1];
                const phaseTop = phase.top;
                const phaseBottom = nextPhase ? nextPhase.top : documentEnd;
                
                if (scrollTop >= phaseTop && scrollTop < phaseBottom) {
                    // Currently in this phase
                    currentPhase = phase.name;
                    const phaseHeight = phaseBottom - phaseTop;
                    
                    if (phaseHeight > 0) {
                        const progressInPhase = (scrollTop - phaseTop) / phaseHeight;
                        accumulatedWeight += phase.weight * Math.min(Math.max(progressInPhase, 0), 1);
                    } else {
                        accumulatedWeight += phase.weight;
                    }
                    break;
                } else if (scrollTop >= phaseBottom) {
                    // Completely past this phase
                    accumulatedWeight += phase.weight;
                }
            }
        }

        // Calculate percentage based on weighted progress (with one decimal place for precision)
        const rawPercent = totalPhaseWeeks > 0 ? (accumulatedWeight / totalPhaseWeeks) * 100 : 0;
        const percentValue = Math.min(Math.max(Math.round(rawPercent * 10) / 10, 0), 100);

        // Update the progress bar
        progressBar.style.width = percentValue + '%';

        // Update the progress percentage text
        progressPercent.textContent = (isNaN(percentValue) ? 0 : percentValue) + '%';

        // Add tooltip showing current phase
        if (currentPhase) {
            progressPercent.title = `Currently viewing: ${currentPhase}`;
        }

        // Hide when at top (0%), show otherwise
        if (percentValue === 0 || isNaN(percentValue)) {
            progressPercent.classList.add('hidden');
        } else {
            progressPercent.classList.remove('hidden');
        }
...
// Initialize progress bar on load and after a short delay to ensure DOM is ready
document.addEventListener('DOMContentLoaded', function() {
    updateProgressBar();
    // Update again after a delay to ensure all content is loaded
